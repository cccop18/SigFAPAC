...
import { PesquisadorEsDtoService } from '../../../service/pesquisador-es-dto.service';
...
@Component({
  selector: 'app-pesquisador-es-form',
  standalone: true,
  ...
  templateUrl: './pesquisador-es-form.component.html',
  styleUrl: './pesquisador-es-form.component.css'
})
export class PesquisadorEsFormComponent implements IForm<PesquisadorEsDto> {
  ...
  constructor(
    private servico: PesquisadorEsDtoService,
  ...
  ngOnInit(): void {
    ...
    if (id) {
      this.servico.getById(+id).subscribe({
        next: (resposta: PesquisadorEsDto) => {
          this.registro = resposta;
          this.formPesquisadorEs.patchValue(this.registro);
        }
      });
    }
  }
  registro: PesquisadorEsDto = <PesquisadorEsDto>{};
  ...
  formPesquisadorEs = new FormGroup({
    ...
    passaportePesquisador: new FormControl<string | null>(null),
    ...
    idTipoPesquisador: new FormControl<number>(2),
  ...
  get form() {
    return this.formPesquisadorEs.controls;
  }
 ...
 // Método para verificar o CEP e preencher os campos de endereço automaticamente
  verificarCep(tipoEndereco: 'RESIDENCIAL' | 'PROFISSIONAL'): void {
    ...
    const formEndereco = tipoEndereco === 'RESIDENCIAL'
      ? this.formPesquisadorEs.get('cepEnderecoResidencial')
      : this.formPesquisadorEs.get('cepEnderecoProfissional');
      ...
      this.cepService.consultarCep(cep).subscribe({
        next: (dados) => {
          ...
          if (dados.erro) {
            ...
            this.formPesquisadorEs.patchValue(enderecoCampos);
          }
        },
        error: () => {
          formEndereco?.setErrors({ cepInvalido: true });
  ...
  save(): void {
    this.registro = Object.assign(this.registro, this.formPesquisadorEs.value);
